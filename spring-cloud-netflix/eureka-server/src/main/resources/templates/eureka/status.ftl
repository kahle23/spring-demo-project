...
<!--[if gt IE 8]><!--> <html class="no-js"> <!--<![endif]-->
  ...
  <body id="one">
                ...
                <td>
                  <#list app.instanceInfos as instanceInfo>
                    <#if instanceInfo.isNotUp>
                      <span style="font-weight:bolder; color:red;"><b>
                    </#if>
                    ...
                    <#list instanceInfo.instances as instance>
                      <#if instance.isHref>
                        ...
                        <#if !(instanceInfo.isNotUp)>
                          - <span onclick="offline('${app.name}/${instance.id}')" style="font-weight:bolder; background-color:red;">OFFLINE</span>
                        </#if>
                        <br>
                      <#else>
                        ${instance.id}
    ...
    <script>
       function offline(serviceName) {
         if (confirm('Do you want to take service "' + serviceName + '" offline?')) {
           $.ajax({
             type: 'delete',
             url: '/eureka/apps/' + serviceName,
             contentType: "application/x-www-form-urlencoded;charset=utf-8",
             success: function () {
               alert("The service successful offline. ");
               location.reload();
             },
             error: function () {
               alert("The service failed offline. ")
             }
           });
         }
       }
